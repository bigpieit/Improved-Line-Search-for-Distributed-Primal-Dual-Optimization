...
static int compute_fun(double* w, double* alpha, double C, int L2, const problem *prob)
{
	...
	info("Iter %d Dual %20.15e Primal %20.15e Time %g Accuracy %g gap %f\n", iter, dual, f, double(total_comp_time.tv_sec) + double(total_comp_time.tv_nsec)/double(1000000000), acc*100,f+dual);
	//info("%d %20.15e %20.15e %20.15e %g\n",iter,dual,f,f+dual,double(total_comp_time.tv_sec) + double(total_comp_time.tv_nsec)/double(1000000000));
	...
	double Cp, double Cn, int solver_type, double beta)
{
	...
	double *deltaw = new double[w_size];
	double lambda = 0;
	schar *y = new schar[l];
	int line_search= 0;
	int converged = 0;
	...
	while (!converged && out_iter < max_iter)
	{
		...
		for (i=0;i<w_size;i++)
			{
				deltaw[i] = w[i] - w_orig[i];    //get every detlaw 
				allreduce_buffer[i] = deltaw[i];
			}
		...
		double *alldeltaw;	
		alldeltaw= (double *)malloc(k*reduce_length*sizeof(double));

		MPI_Allgather(allreduce_buffer,reduce_length,MPI_DOUBLE,alldeltaw,reduce_length,MPI_DOUBLE,MPI_COMM_WORLD);
		for (int j=0;j<reduce_length;j++)
			allreduce_buffer[j]=0;
		for (int j=0;j<reduce_length;j++)
		{
			for (i=0;i<k;i++)
				allreduce_buffer[j]+=alldeltaw[j+i*reduce_length];
		}
		...
		double sum_alpha_inc=0;
		for (i=0;i<w_size;i++)
			pk += w_orig[i]*deltaw[i];

		pk=pk-das;                                     
		double *allpk;
		if (mpi_get_rank()==0)
		allpk=(double *)malloc(k*sizeof(double));
		
		MPI_Gather(&pk,1,MPI_DOUBLE,allpk,1,MPI_DOUBLE,0,MPI_COMM_WORLD);

		double *rowofMK=new double[k];
		for (i=0;i<k;i++)
			rowofMK[i]=0;   //initialize
		for (int j=0;j<k;j++)
			for (i=0;i<w_size;i++)
				rowofMK[j]+=deltaw[i]*alldeltaw[j*reduce_length+i]; //get each row
		double *mm; 
		if (mpi_get_rank()==0)
		{
		mm= (double *)malloc(k*k*sizeof(double));//matrix M
		}
		MPI_Gather(rowofMK,k,MPI_DOUBLE,mm,k,MPI_DOUBLE,0,MPI_COMM_WORLD);

		double *betaall;
		if (mpi_get_rank()==0)
		{
		betaall= (double *)malloc(k*sizeof(double));
		double *mball;
		mball= (double *)malloc(k*sizeof(double));

		for (i=0;i<k;i++)
			betaall[i]=eta;          //initialize all beta to eta
		
		double stepsize=0.000001;

		int tempiter=0;
		while (tempiter<1000)    //gradient descent to solve the all beta
		{			
			tempiter++;
			for (i=0;i<k;i++)
				mball[i]=0;
			for (int j=0;j<k;j++)
				for (i=0;i<k;i++)
					mball[j]+=mm[i+j*k]*betaall[i];

			for (int j=0;j<k;j++)
			{
				betaall[j]=betaall[j]- stepsize*(mball[j]+allpk[j]);
				betaall[j]=min(max(betaall[j],0.0),1.0);               
			}
		...
		MPI_Scatter(betaall,1,MPI_DOUBLE,&betak,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
		double *betadelta=new double[w_size];
		for (i=0;i<w_size;i++)
			betadelta[i]=deltaw[i]*betak;
		mpi_allreduce(betadelta,w_size,MPI_DOUBLE,MPI_SUM);

		double tttbegin = clock();

		int num_s=l;
		int k=int(mpi_get_size());
		double wdx[l];
		double wx[l];
		double flag[l];
		for (int i=0;i<l;i++)
		...
		for (int i=0;i<l;i++)          //random from 0----n-1
		{	
			int countfori=0;
			int j=rand()%num_s;
			countfori+=1;
			...
			while(xi->index!= -1)
			{
				wdx[j] += betadelta[xi->index-1]*(xi->value); 
				wx[j]+=w_orig[xi->index-1]*(xi->value);
				flag[j]=1;
				xi++;
			}
			if (i > 60)
		...
		for (int i=0;i<w_size;i++)
				reg_ori += (w_orig[i]+min_eta*allreduce_buffer[i])*(w_orig[i]+min_eta*allreduce_buffer[i]);
		for (int i=0;i<l;i++)
			{
				if (flag[i]==1)
				{l_ori[i]=wx[i]+min_eta*wdx[i];
				l_ori[i]=1-l_ori[i]*prob->y[i];
				if (l_ori[i]>0)
					if (L2)
						xi_ori += l_ori[i]*l_ori[i];
					else
						xi_ori+=l_ori[i];
				}
			}
		...
		for (int i=0;i<l;i++)
			{
				dual_ori-=alpha_orig[i]+min_eta*alpha_inc[i];
			}
		
		xi_ori=xi_ori/60*l;
		mpi_allreduce_notimer(&xi_ori, 1, MPI_DOUBLE, MPI_SUM);
		mpi_allreduce_notimer(&dual_ori, 1, MPI_DOUBLE, MPI_SUM);
		double p_ori = reg_ori / 2.0  + xi_ori*C;  
		dual_ori =  reg_ori / 2.0 +dual_ori*C;  

		while (eta > 1.0/k)               //for gurantee n
		{
			double xi_new = 0;
			...
			for (int i=0;i<w_size;i++)
			{
				reg += (w_orig[i]+eta*betadelta[i])*(w_orig[i]+eta*betadelta[i]);
			}		                // lambda needs to be input.        MODIFIED　LATER
			for (int i=0;i<l;i++)
			{
				if (flag[i]==1)
				{templ[i]=wx[i]+eta*wdx[i];
				templ[i]=1-templ[i]*prob->y[i];
				if (templ[i]>0)
					if (L2)
						xi_new += templ[i]*templ[i];
					else
						xi_new+=templ[i];
				}
			}
			...
			for (int i=0;i<l;i++)
			{
				...
				dual_new-=alpha_orig[i]+eta*betak*alpha_inc[i];
			}
			xi_new=xi_new/60*l;
			mpi_allreduce_notimer(&xi_new, 1, MPI_DOUBLE, MPI_SUM);
			mpi_allreduce_notimer(&dual_new, 1, MPI_DOUBLE, MPI_SUM);
			double p = reg / 2.0  + xi_new*C; 	           	
			dual_new =  reg / 2.0 +dual_new*C;           
			if  (eta == 1.0/k)
				break;
			if ((p+dual_new)<(p_ori+dual_ori))
				break;
			else
				eta=eta/2;
			//when initialized the first w_orig[i] will be all zero, thus, the new will always larger than old(0)

		}
		for (i=0;i<w_size;i++)
			w[i] = w_orig[i] + eta*betadelta[i];						//Stochastic BlockLS.
		for (i=0;i<l;i++)
			alpha[i] = alpha_orig[i] + eta*betak * alpha_inc[i];   
